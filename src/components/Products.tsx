			...
			{products.map((product) => {
				...
				return (
					...
					>
						...
						{qty === 0 ? (
							<button className="bg-black text-white rounded w-full py-1.5 sm:py-2 mt-auto font-semibold text-xs sm:text-base transition hover:bg-gray-800" onClick={() => onBuy(product.id)}>
								купить
							</button>
						) : (
							<div className="flex items-center gap-1 sm:gap-2 mt-auto w-full justify-center">
								<button className="bg-black text-white px-2 sm:px-3 py-1 rounded text-base sm:text-lg font-bold" onClick={() => onDecrement(product.id)}>-</button>
								<input
									type="number"
									min={1}
								...
								/>
								<button className="bg-black text-white px-2 sm:px-3 py-1 rounded text-base sm:text-lg font-bold" onClick={() => onIncrement(product.id)}>+</button>
							</div>
						)}
					</div>
				);
